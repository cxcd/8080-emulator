...
void add(state *s, uint8_t &reg1, uint8_t &reg2, uint8_t &reg3, uint8_t &reg4) {
	uint32_t hl = (reg1 << 8) | reg2;
	uint32_t bc = (reg3 << 8) | reg4;
...
void emulate8080(state *s) {
	...
	case 0x02: // STAX B
		...
		break;
	case 0x03: // INX B
		s->offset = (((uint16_t)s->r.b << 8) | s->r.c) + 1;
		s->r.b = s->offset >> 8;
		s->r.c = s->offset & 0xFF;
		break;
	case 0x04: // INR B
	...
	case 0x12: // STAX D
		...
		break;
	case 0x13: // INX D
		s->offset = (((uint16_t)s->r.d << 8) | s->r.e) + 1;
		s->r.d = s->offset >> 8;
		s->r.e = s->offset & 0xFF; 
		break;
	case 0x14: // INR D
